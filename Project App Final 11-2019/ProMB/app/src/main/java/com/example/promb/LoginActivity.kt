    ...
    fun Login(v: View){
        ...
        api.loginUser(
            ...
            override fun onResponse(call: Call<User>,response: Response<User>){
                    userList.add(User(response.body()?.user_id.toString().toInt(),
                        response.body()?.username.toString(),
                        response.body()?.password.toString(),
                        response.body()?.type.toString().toInt(),
                        response.body()?.email.toString(),
                        response.body()?.name.toString()
                        ))
                if (response.isSuccessful()){
                    Toast.makeText(applicationContext,"Login Successfull !!",Toast.LENGTH_SHORT).show()
                    intent.putExtra("mType",response.body()?.type.toString())
                    intent.putExtra("mUid",response.body()?.user_id.toString())
